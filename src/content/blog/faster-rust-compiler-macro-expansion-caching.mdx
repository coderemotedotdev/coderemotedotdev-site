...
**The bad news:** These options are of limited help with respect to incremental build times.
  ...
  - Option 5 brings us intraprocess parallelization. It can significantly speed up clean dev builds[^parallel-frontend] -- provided of course that your machine has that enough CPU and memory. That said, personally I have noticed negligible improvement in incremental builds because the parallelized 'analysis' phase is far shorter than that in clean builds.
...
3. Cache the AST and "push" changes: Assuming small edits between builds, the AST (Abstract Syntax Tree) that represents a crate changes very slightly. Nevertheless, the compiler currently lexes and parses and expands from scratch.[^technical-macro-exp] My vague idea here was to cache the AST from the last run, identify the code changes in the new version, and push those AST changes to the previous crate's AST. This felt like it was on the right track but the devil is in the details:
    ...
    - Are parsing and lexing even time-consuming i.e. do we benefit from caching them?
</details>
...
Finally, I landed at caching procedural macro expansion. The core idea is simple. Procedural macros take as input a `TokenStream` (representing an AST) and output another `TokenStream`. We cache this mapping from our current build; if the input is unchanged in our next build, we reuse that output instead of recomputing it.

This is far superior to "using release builds of procedural macro crates" [mentioned before](#existing-compiler-options-offer-limited-help-with-incremental-build-times). For the vast majority of macros, instead of doing the same work faster, we are doing *zero work*.
...
We needed to augment the compiler in several ways to support macro expansion caching. In particular, we needed to be able to do the following:
  ...
  2. Hash macro (bang, attribute, and derive) invocations. Our cache is a hashmap mapping invocation hashes to the expansions. As in the other areas of the compiler, we need to take care to use `StableHash`es, so that the hashes are consistent across runs. On this, we avoided hashing `span`s (which represent positions in code) where possible because ideally our cache remains valid even if a new line of code pushes the code below it down a line.
  3. Handle edge cases: duplicate invocation hashes, interpolated tokens[^interpolated-tokens], and nested macros. All of these poses challenges to computing unique invocation hashes and consistent output ASTs. We typically flattened ASTs where possible and dropped un-cacheable ones where not.
  4. Serialize macro output. Luckily, we were able to leverage a lot of the data structures in place for incremental builds downstream in the compiler. Still, we needed to spin our own serialization mechanism since we are newly caching `TokenStreams` and ASTs.
...
fn expand_macro_invoc(
  ...
  let tok_result: TokenStream = {
    // 1. Perform lookup of the macro invocation hash in our new macro cache
...
| Project      | speed up % | Default build time (incremental) | Modded build time (incremental) | Detailed analysis?
| ------------- | ------------- | ------------- | ------------- | ------------- |
...
|  <a href="https://github.com/vectordotdev/vector" target="_blank">vector</a> | 11% | 18.4 s | 16.4 s | <a href="https://github.com/vectordotdev/vector/discussions/19930" target="_blank">In vector's GitHub discussion</a>

#### Proof by picture
As you can see in this `rustc` profile, we are expanding only the procedural macros that we modified. 
...
But if you want to return minutes of your life with every compile, check out what we're building at CodeRemote.
